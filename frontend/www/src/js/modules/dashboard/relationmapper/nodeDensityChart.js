...
class NodeDensityChart extends HTMLElement {
    ...
    connectedCallback() {
        this.subscribePanListener();
    }

    subscribePanListener() {
        this.unsubscribePanListener = broadcast.subscribe( 'relationGraphPanZoom', (eventData) => {
            this.showPanPosition( eventData.panXRatio, eventData.panExtentRatio );
        } );
    }
    ...
    render(readingData, options = {}) {
        ...
        const sectionXs = sectionStartXs.map((d, idx) => {
            return {
        ...
        chartG
            ...
            .on('click', (evt, d) => {
                ...
                // In the case we need to switch sections we use a temporary listener that will react on the 
                // next first `relationGraphPanZoom` event (which happens when the new section has been 
                // rendered). We then first remove the listener and then broadcast a `minimapPan` event, which 
                // is picked up by `relationRenderer` which pans the graph to the right position and broadcasts
                // another `relationGraphPanZoom` to which `this.showPanPosition` reacts and redraws the pan
                // position indicator in the right place in the minimap.
                //     Essentially this is an event based way to remember the `xRatio` computed here until it's 
                // needed. And yes, we could also store it in this.#ratio and null that in `showPanPosition`
                // when used, but it's this click event responsibility and it feels less prone to coding error.
                //     Note: to prevent the first/default `relationGraphPanZoom` handler/listener from moving  
                // the pan position indicator to the start of the section in the minimap we switched to, we 
                // remove that one first and add it back after we're done.
                if (d.section != SECTION_STORE.state.selectedSection.id) {
                    this.unsubscribePanListener();
                    const unsubscribeTmpListener = broadcast.subscribe( 'relationGraphPanZoom', (eventData) => {
                        unsubscribeTmpListener();
                        this.subscribePanListener(); 
                        broadcast.publish( 'minimapPan', { panXRatio: xRatio } );
                    } );
    ...
    // We may receive motifications of panning already before the basic
    // density chart had been rendered. In that case we wait until we
    // know there is a section part (svg rectangle) available.
    getSectionRectangle( sectionId ) {
    ...
    showPanPosition(panXRatio, extentRatio) {
        ...
        this.getSectionRectangle( sectionId ).then( (rectElement) => { 
            ...
            indicator.call(d3.drag().on('drag', (event) => {
                ...
                if ((event.x > minX) && (event.x < maxX)) {
                    ...
                    if ((currentMinX + event.dx > minX) && ((currentMaxX + event.dx) < maxX)) {
                        ...
                        // Broadcast the drag to whatever component might be interested 
                        // (probably the relationRenderer rendering the relation graph).
                        broadcast.publish( 'minimapPan', { panXRatio: xRatio, silent: true } );
                    }
                }
            }));
        } )
