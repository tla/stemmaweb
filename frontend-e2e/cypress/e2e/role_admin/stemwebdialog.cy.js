...
// if (Cypress.browser.isHeaded) {
  ...
  describe('Stemweb dialog should work properly', () => {
    it('passes', () => {
      ...
      cy.get('@stemwebmodal')
        .contains('Generate a Stemweb tree')
        .should('be.visible');
      ...
      cy.get('@stemwebmodal')
        ...
        .each(($el, index, $list) => {
          ...
          cy.get('@stemwebmodal')
            .find('select option:selected') // get the selected option
            .invoke('text')
            .then((txt) => {
              ...
              cy.get('@stemwebmodal')
                .find('form')
                .find('svg') // .its('length').then((len) => {cy.log(len)});
              ...
              cy.get('@stemwebmodal')
                ...
                .invoke('html')
                .then((innerHTML) => {
                  expect(innerHTML).equal(
                    stemweb_algorithms[index].description
                  );
                });
            ...
            if (algorithm.text === optionText) {
              ...
              if (optionText === 'Pars') {
                ...
                cy.get('@stemwebmodal')
                  ...
                  .contains('Run')
                  .wait(500)
                  .click(); // wait() for the event listener to close the modal to be attached (https://www.cypress.io/blog/2019/01/22/when-can-the-test-click)
    ...
    it('passes', () => {
      test_traditions.forEach((tradition) => {
        ...
        if (traditionTitle.startsWith('Florilegium')) {
          ...
          cy.get('stemweb-job-status')
            .contains('#job_status', '2')
            .and('contain.text', 'Running');
          ...
          cy.get('stemweb-job-status')
            .contains('#job_status', '3')
            .and('contain.text', 'Error')
            .and('contain.text', 'Pretend we had an error here.');
    ...
    it(
      ...
      () => {
        const tradition = test_traditions.find((trad) =>
          trad.title.startsWith('Florilegium')
        );
        ...
        cy.get('#stemma-editor-graph-container')
          .find('#stemma-selectors')
          .find('svg.feather-file')
          .should('have.length', tradition.stemmata.length);
        ...
        cy.get('@editorbox')
          .find('textarea#stemma-dot-editor')
          .invoke('val')
          .then((v) => {
            cy.log('old val: ' + v);
            ...
            cy.get('textarea#stemma-dot-editor')
              .type('{moveToEnd}{leftArrow}' + appendatend)
              .wait(1000);

            // get the graph's svg again and assert the number of its edges to be one more than before
            cy.get('div#graph > svg')
              .find('g.edge')
              .should('have.length', countedges + 1); // 21
  ...
  describe('stemma editing error feedback in message console works properly', () => {
    it('passes', () => {
      ...
      let tradition = test_traditions.find((trad) =>
        trad.title.startsWith('Notre besoin')
      );
      cy.log('tradition.title: ' + tradition.title);
      // click on the tradition title within the tradition list
      cy.get('#traditions-list').contains(tradition.title).click();
      ...
      cy.get('@editorbox')
        ...
        .then((v) => {
          ...
          cy.get('textarea#stemma-dot-editor')
            .type('{selectAll}{backspace}' + faultydot)
            .wait(1000);
          ...
          const msg_err =
            'Error: BAD REQUEST; Witness ' +
            witness +
            ' not marked as either hypothetical or extant';
          ...
          tradition = test_traditions.find((trad) =>
            trad.title.startsWith('Verbum')
          );
          cy.log('tradition.title: ' + tradition.title);
          // click on the tradition title within the tradition list
          cy.get('#traditions-list').contains(tradition.title).click();
