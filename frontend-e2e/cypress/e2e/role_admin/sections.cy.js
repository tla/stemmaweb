...
if (Cypress.browser.isHeaded) {
    ...
    it('passes', () => {
      ...
      test_traditions.forEach((tradition) => {
        ...
        cy.get('@navitem')
          ...
          .find('ul')
          .children()
          .should('have.length', tradition.sectionscount);
      });
    });
  });
  ...
  describe('Adding a section via text directory feather-plus-circle, and deleting it via section properties bin works', () => {
    it.skip('passes', () => {
      ...
      const new_section_rel_path = Cypress.browser.isHeaded
        ? './../bin/init-data/stemmarest/data/florilegium_z.csv'
        : './cypress/.initdata4headless/data/florilegium_z.csv';
      ...
      test_traditions
        .filter(({ title }) => title === 'Florilegium "Coislinianum B"')
        .forEach((tradition) => {
          ...
          cy.contains('#sidebar-menu', 'Text directory')
            .find('svg.feather-plus-circle')
            .click();
          cy.get('.modal-content').find('#button_new_section').click();
          cy.get('#add_tradition_modal')
            .as('add_tradition_or_section')
            .should('be.visible');
          cy.get('@add_tradition_or_section')
            .find('input#new_name')
            .click()
            .wait(500)
            .type(new_section_name, { delay: 50 });
          cy.get('@add_tradition_or_section')
            .find('input#uploadfile')
            .selectFile(new_section_rel_path);
          cy.get('@add_tradition_or_section')
            .find('select#new_filetype')
            .select('Comma-separated values (spreadsheet collation)')
            .should('have.value', 'csv');
          cy.get('@add_tradition_or_section')
            .find('select#upload_for_tradition')
            .select('Florilegium "Coislinianum B"');
      ...
      () => {
        ...
        test_traditions
          ...
          .forEach((tradition) => {
            ...
            cy.get('@sectionmodal')
              .contains('Edit section properties')
              .should('be.visible');
            ...
            cy.get('@sectionmodal')
              ...
              .type('{backspace}')
              .wait(500)
              .type(section_name_new, { delay: 50 })
              .wait(500);
            // press 'Save'
            cy.get('@sectionmodal')
              .find('button')
            ...
            cy.get('section-list')
              .contains(section_name_orig)
              .should('not.exist'); // failed on github because the new content was appended instead of replacing the orig --> added wait() after selecting orig name
            ...
            cy.get('@sections').each(($ele, index) => {
              if ($ele.text().trim() == section_name_new) {
                expect(tradition.sections[index].name).not.to.eq(
                  $ele.text().trim()
                );
              } else {
                expect(tradition.sections[index].name).to.eq(
                  $ele.text().trim()
                );
              }
            });
